...
export class ProdutosPage {
  ...
  ionViewWillEnter() {
    ...
    this.categoriaSelecionada = <CategoriaModel>(
      JSON.parse(localStorage.getItem(ConfigHelper.storageKeys.selectCategory))
    );
    this.load();
  }
  ...
  async load(): Promise<void> {
    try {
      let produtosResult = await this.produtoSrv.produtosByCategoriaId(
        this.categoriaSelecionada._id
      );
